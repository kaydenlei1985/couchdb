...
new_interactive_index(Db, Mrst, VS) ->
    ...
    couch_views_fdb:set_build_state(Db, Mrst, ?INDEX_BUILDING).
...
get_build_state(TxDb, #mrst{sig = Sig}) ->
    #{
        tx := Tx
    } = TxDb,
    Key = build_state_key(TxDb, Sig),
    erlfdb:wait(erlfdb:get(Tx, Key)).


set_build_state(TxDb, #mrst{sig = Sig}, State) ->
    #{
        tx := Tx
    } = TxDb,
